...
//o () é usado para dar prioridade à operação e só atribuí-la ao lancamento após sua conclusão
let estrelas = [];
for (let i = 0; i < 500; i++){
    estrelas[i] = {
        x: Math.random() * canvas.width,
        y: Math.random() * canvas.height,
        raio: Math.sqrt(2 * Math.random()),  //sqrt = raiz quadrada
        brilho: 1.0,
        apagando: true,
        cintilacao: 0.05 * Math.random()
    }
}
//Serão criados vários objetos com todos os valores entre as chaves

let moduloLunar = {
    posicao: {
        x: lancamento ? 100 : 700,//operador ternário ? resultado se verdadeiro : resultado se falso
        y: 100
    },
...
function mostrarAngulo(){
    mostrarIndicador(
        ...
        //fórmula de conversão de radianos para grau
        400,
        40
    );
}
...
function mostrarAltitude(){
    mostrarIndicador(
        `Altitude: ${(canvas.height - moduloLunar.posicao.y - 
            0.5 * moduloLunar.altura ).toFixed(0)}m`,
        400,
        60
    );
}
...
function encerrarJogo(){
    if(moduloLunar.posicao.y > canvas.height - moduloLunar.altura* 0.5){
        if(moduloLunar.velocidade.y <= 0.5 &&
            ...
            mostrarResultado("Troque de emprego!", cor = "red");
        }
        return true;
    }
    return false;
